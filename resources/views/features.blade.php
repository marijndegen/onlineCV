<!-- Start feature Area -->
<section class="feature-area pt-20 pb-50">
    <div class="container">
        <div class="row d-flex justify-content-center">
            <div class="menu-content col-lg-8">
                <div class="title text-center">
                    <h1 class="mb-10">{{ trans('index.skillAndProject') }}</h1>
                </div>
            </div>
        </div>
        <div class="row">
            <div class="col-lg-4 col-md-6 ">
                <div class="single-feature mb-30">
                    <div class="title d-flex flex-row pb-20">
                        <span class="lnr lnr-car"></span>
                        <h4>{{trans('index.toyotaStarletAutoStartHeading') }}</h4>
                    </div>
                    <p>
                        {{ trans('index.toyotaStarletAutoStart')}}
                        <br><br>
                        <a href="https://github.com/marijndegen/toyotastarletautostart" target="_blank">{{ trans('index.viewOnGitHub') }}</a>
                    </p>
                </div>
            </div>
            <div class="col-lg-4 col-md-6 ">
                <div class="single-feature mb-30">
                    <div class="title d-flex flex-row pb-20">
                        <span class="lnr lnr-layers"></span>
                        <h4>{{ trans('index.ethereumBlockChainProjectHeading')}}</h4>
                    </div>
                    <p>
                        {{ trans('index.ethereumBlockChainProject')}}
                        <br><br>
                        <a href="https://github.com/marijndegen/Blockchainproject2018allRepos" target="_blank">{{trans('index.viewOnGitHub')}}</a>
                    </p>
                </div>
            </div>
            <div class="col-lg-4 col-md-6 ">
                <div class="single-feature mb-30">
                    <div class="title d-flex flex-row pb-20">
                        <span class="lnr lnr-paperclip"></span>
                        <h4>{{ trans('index.onlineCVHeading')}}</h4>
                    </div>
                    <p>
                        {{ trans('index.onlineCV')}}
                        <br><br>
                        <a href="https://github.com/marijndegen/onlineCV" target="_blank">{{trans('index.viewOnGitHub')}}</a>
                    </p>
                </div>
            </div>
            <div class="col-lg-4 col-md-6 ">
                <div class="single-feature">
                    <div class="title d-flex flex-row pb-20">
                        <span class="lnr lnr-lock"></span>
                        <h4>{{ trans('index.rsaKeyPairEncrypterHeading')}}</h4>
                    </div>
                    <p>
                        {{ trans('index.rsaKeyPairEncrypter')}}
                        <br><br>
                        <a href="https://github.com/marijndegen/laravel-Keypair-admin" target="_blank">{{trans('index.viewOnGitHub')}}</a>
                    </p>
                </div>
            </div>
            <div class="col-lg-4 col-md-6 ">
                <div class="single-feature">
                    <div class="title d-flex flex-row pb-20">
                        <span class="lnr lnr-keyboard"></span>
                        <h4>{{ trans('index.multiChatAppHeading')}}</h4>
                    </div>
                    <p>
                        {{ trans('index.multiChatApp')}}
                        <br><br>
                        <a href="https://github.com/marijndegen/MultiChat" target="_blank">{{trans('index.viewOnGitHub')}}</a>
                    </p>
                </div>
            </div>
            <div class="col-lg-4 col-md-6 ">
                <div class="single-feature">
                    <div class="title d-flex flex-row pb-20">
                        <span class="lnr lnr-clock"></span>
                        <h4>{{ trans('index.javascriptSnakeHeading') }}</h4>
                    </div>
                    <p>
                        {{ trans('index.javascriptSnake') }} <a href="https://snake.marijndegen.nl">snake.marijndegen.nl</a>
                        <br><br>
                        <a href="https://github.com/marijndegen/javascript-snake" target="_blank">{{trans('index.viewOnGitHub')}}</a>
                    </p>
                </div>
            </div>
        </div>
    </div>
</section>
<!-- End feature Area -->